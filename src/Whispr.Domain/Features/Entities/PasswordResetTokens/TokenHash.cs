...
public sealed record TokenHash : ValueObject
{
    ...
    public static TokenHash Create(string rawHash)
    {
        ...
        Ensure.MinimumLength(
            rawHash,
            Constants.Constraints.PasswordResetToken.TokenLength,
            $"TokenHash hash must be at least {Constants.Constraints.PasswordResetToken.TokenLength} characters.", nameof(rawHash));
